...
export default function Sidebar({ suborgan }: { suborgan: suborganName[] }) {
	const [modal, setModal] = useState(false);
	const [isFullscreen, setIsFullscreen] = useState(false);
	const [hasilVoteOpen, setHasilVoteOpen] = useState(false);
	const [kandidatOpen, setKandidatOpen] = useState(false);
							...
							<div className="flex-1 space-y-12 max-h-[60px] w-full bg-white px-3w">
								...
								<ul className="space-y-4 pb-2">
									...
									<li>
										<div>
											<button
												onClick={() => setKandidatOpen(!kandidatOpen)}
												className="group flex items-center justify-between w-full rounded-[50px] px-5 py-3 text-base font-normal text-primary-400 hover:bg-secondary-color hover:text-white transition-all"
											>
												<div className="flex items-center">
													<KandidatIcon />
													<span className="ml-3 whitespace-nowrap text-primary-400 font-semibold">
														Kandidat
													</span>
												</div>
												<svg
													className={`w-4 h-4 transition-transform ${
														kandidatOpen ? "rotate-180" : ""
													}`}
													fill="none"
													viewBox="0 0 24 24"
													stroke="currentColor"
											...
											<div
												className={`overflow-hidden transition-all duration-300 ease-in-out ${
													kandidatOpen ? "max-h-96" : "max-h-0"
												}`}
											>
												<ul className="pl-8 space-y-2 mt-2">
													{suborgan.map((item, index) => (
														<div key={index}>
															<Link
																href={`/admin/candidates/${item.id}`}
																className="group flex items-center rounded-[50px] px-5 py-2 text-base font-normal text-primary-400 hover:bg-secondary-color hover:text-white transition-all"
															>
																{item.name}
															</Link>
														</div>
									...
									<li>
										<button
											onClick={() => setHasilVoteOpen(!hasilVoteOpen)}
											className="group flex items-center justify-between w-full rounded-[50px] px-5 py-3 text-base font-normal text-primary-400 hover:bg-secondary-color hover:text-white transition-all"
										>
											<div className="flex items-center">
												<HasilVote />
										...
										<div
											className={`overflow-hidden transition-all duration-300 ease-in-out ${
												hasilVoteOpen ? "max-h-96" : "max-h-0"
											}`}
										>
											<ul className="pl-8 space-y-2 mt-2">
												{suborgan.map((item, index) => (
													<li key={index}>
														<Link
															href={`/admin/hasilVote/${item.name.toLowerCase()}`}
															className="group flex items-center rounded-[50px] px-5 py-2 text-base font-normal text-primary-400 hover:bg-secondary-color hover:text-white transition-all"
														>
															{item.name}
														</Link>
